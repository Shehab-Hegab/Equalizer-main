...
# ---------------------------
    ...
    def update_output_plot(self):
        ...
        x_min = max(0, x_max - 10)
        self.curves[1].setData(x_data[:self.ptrs[1] + 2], y_data[:self.ptrs[1] + 2])
    ...
    def eq_slider(self, index):
        ...
        self.data_modified_fft = self.multiply_fft( 
            self.output[2], self.data_ranges[index], gain, self.MultModeCombobox.currentText())
    ...
    def multiply_fft(self, data, indices, gain, mult_mode):
        modified_data = data.copy()
        window_size = len(modified_data[indices])

        gain_factor, _ = self.multiply_fn(mult_mode, window_size, gain)
    ...
    def check_mult_mode(self, combobox_text):
        ...
        fft_freq = np.abs(self.output[3])
        fft_values = np.abs(self.output[2])

        self.plot_widgets[3].clear()
        self.plot_widgets[3].plot(fft_freq, fft_values)

        windows = np.zeros_like(fft_values)

        for i in range(self.enabled_sliders[self.ModeCombobox.currentText()]):
            indices = self.data_ranges[i]
            
            if len(indices) > 0:
                amp = np.abs(self.output[2][indices]).max()

                window, visible_bool = self.multiply_fn(combobox_text, len(indices), amp)
                
                windows[indices] += window

        self.StdSlider.setVisible(visible_bool)

        self.plot_widgets[3].plot(fft_freq, windows, pen=pg.mkPen(color='r'), fillLevel=-0.3, brush=(50,50,200,50))

        self.MultModePlot.plot(fft_freq, windows)


        # Update the plot
        self.plot_widgets[3].autoRange()



    def multiply_fn(self, mult_mode, window_size, amplitude):
        visible_bool = False
        if mult_mode == 'Rectangle':
            window = amplitude * np.ones(window_size)
        elif mult_mode == 'Hamming':
            window = amplitude * np.hamming(window_size)
        elif mult_mode == 'Hanning':
            window = amplitude * np.hanning(window_size)
        elif mult_mode == 'Gaussian':
            sigma = self.StdSlider.value()
            window = amplitude * np.exp(-0.5 * ((np.arange(window_size) - (window_size) / 2) / sigma) ** 2)
            visible_bool = True
        
        return window, visible_bool
    ...
    def sliders_adjustment(self):
        ...
        if self.ModeCombobox.currentText() == "Uniform Range Mode":
            for n in range(10):
                low_band = self.band_length * n #0
                high_band = self.band_length * (n + 1) #2204
